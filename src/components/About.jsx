...
import { Code2, GraduationCap, Trophy, Star, ChevronDown, ChevronUp, Database, Server, Globe, Palette, Box, Layers, Target, Heart, Zap, Award, BookOpen, TrendingUp, Brain, Rocket, Coffee, Monitor, Smartphone, TabletSmartphone } from 'lucide-react';
...
const AboutSection = () => {
  ...
  const animateStats = () => {
    const targets = { projects: 15, satisfaction: 98, experience: 2, passion: 100 };
    const duration = 2000;
    const steps = 60;
  ...
  const skills = [
    { name: 'React', icon: Globe, level: 95, category: 'Frontend', color: 'from-blue-500 to-cyan-500', desc: 'Advanced component architecture & hooks' },
    { name: 'Node.js', icon: Server, level: 90, category: 'Backend', color: 'from-green-500 to-emerald-500', desc: 'RESTful APIs & server-side logic' },
    { name: 'MongoDB', icon: Database, level: 85, category: 'Database', color: 'from-green-600 to-teal-600', desc: 'NoSQL database design & optimization' },
    { name: 'Express.js', icon: Layers, level: 88, category: 'Backend', color: 'from-gray-600 to-gray-800', desc: 'Middleware & routing expertise' },
    { name: 'Redux', icon: Box, level: 82, category: 'Frontend', color: 'from-purple-500 to-violet-600', desc: 'State management & data flow' },
    { name: 'Tailwind CSS', icon: Palette, level: 92, category: 'Frontend', color: 'from-cyan-500 to-blue-500', desc: 'Responsive design & utility-first CSS' },
    { name: 'JavaScript', icon: Code2, level: 94, category: 'Language', color: 'from-yellow-500 to-orange-500', desc: 'ES6+ features & async programming' },
    { name: 'TypeScript', icon: Code2, level: 78, category: 'Language', color: 'from-blue-600 to-indigo-600', desc: 'Type-safe development' },
    { name: 'Git', icon: Rocket, level: 86, category: 'Tools', color: 'from-orange-500 to-red-500', desc: 'Version control & collaboration' }
  ];

  const skillCategories = ['All', 'Frontend', 'Backend', 'Database', 'Language', 'Tools'];
  ...
  const AboutCard = ({ icon: Icon, title, children, expandable = false, cardId, gradient = "from-slate-800/60 to-slate-900/60" }) => {
    ...
    return (
      <div className={`group relative bg-gradient-to-br ${gradient} backdrop-blur-xl border border-slate-700/50 rounded-2xl lg:rounded-3xl p-4 sm:p-6 lg:p-8 transition-all duration-700 hover:bg-gradient-to-br hover:from-slate-800/80 hover:to-slate-900/80 hover:border-slate-600/70 hover:shadow-2xl hover:shadow-blue-500/10 hover:-translate-y-2 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        
        {/* Enhanced animated border gradient */}
        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/30 via-purple-500/30 to-cyan-500/30 rounded-2xl lg:rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 -z-10 blur-xl"></div>
        
        {/* Multiple glowing orbs */}
        <div className="absolute top-4 right-4 w-12 h-12 bg-gradient-to-br from-blue-400/30 to-purple-500/30 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
        <div className="absolute bottom-4 left-4 w-8 h-8 bg-gradient-to-br from-cyan-400/20 to-blue-500/20 rounded-full blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
        
        <div className="relative z-10">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-4">
            <div className="flex items-center space-x-3 sm:space-x-4">
              <div className="relative">
                <div className="p-2 sm:p-3 lg:p-4 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl lg:rounded-2xl shadow-lg group-hover:shadow-blue-500/25 transition-all duration-300 group-hover:scale-110">
                  <Icon className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 text-white" />
                </div>
                <div className="absolute inset-0 bg-gradient-to-br from-blue-400 to-purple-500 rounded-xl lg:rounded-2xl blur-lg opacity-50 group-hover:opacity-75 transition-opacity duration-300 -z-10"></div>
              </div>
            ...
            {expandable && (
              <button
                ...
                className="self-start sm:self-center p-2 sm:p-3 rounded-lg sm:rounded-xl bg-slate-700/50 hover:bg-slate-600/50 border border-slate-600/30 hover:border-slate-500/50 transition-all duration-300 hover:scale-110 group"
              >
                {isExpanded ? 
  ...
  const SkillCard = ({ skill, index }) => {
    ...
    return (
      ...
      >
        <div className={`relative overflow-hidden p-4 sm:p-5 lg:p-6 bg-gradient-to-br ${skill.color}/10 backdrop-blur-sm border border-white/10 rounded-xl lg:rounded-2xl transition-all duration-500 hover:border-white/30 hover:shadow-xl hover:shadow-blue-500/20 ${isVisible ? 'animate-slide-up' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            ...
            <div className="absolute top-1/2 left-2 w-0.5 h-0.5 bg-purple-400 rounded-full animate-pulse" style={{animationDelay: '1s'}}></div>
          </div>
          
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              <div className="flex items-center space-x-2 sm:space-x-3">
                <div className={`p-2 bg-gradient-to-br ${skill.color} rounded-lg transition-transform duration-300 group-hover:scale-110`}>
                  <skill.icon className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
                </div>
                <span className="text-white font-semibold text-sm sm:text-base lg:text-lg">{skill.name}</span>
              </div>
        ...
        {isHovered && (
          <div className="absolute -top-16 left-1/2 transform -translate-x-1/2 px-3 py-2 bg-slate-900/95 backdrop-blur-sm text-white text-sm rounded-xl shadow-xl z-30 border border-slate-700 whitespace-nowrap">
            {skill.level}% Proficiency • {skill.category}
  ...
  const StatCard = ({ icon: Icon, value, label, suffix = '', gradient, delay = 0 }) => (
    <div className={`group relative transition-all duration-700 delay-${delay} ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
      <div className={`text-center p-4 sm:p-6 lg:p-8 bg-gradient-to-br ${gradient} rounded-2xl lg:rounded-3xl backdrop-blur-sm border border-white/10 group-hover:border-white/20 transition-all duration-500 group-hover:scale-105 group-hover:shadow-2xl h-full`}>
        
        <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent rounded-2xl lg:rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
        
        <div className="relative z-10">
          <div className="p-2 sm:p-3 lg:p-4 bg-white/10 rounded-xl lg:rounded-2xl backdrop-blur-sm mb-3 sm:mb-4 mx-auto w-fit group-hover:bg-white/15 transition-colors duration-300">
            <Icon className="w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8 text-white mx-auto group-hover:scale-110 transition-transform duration-300" />
          </div>
        ...
        <div className="absolute bottom-6 left-6 w-1 h-1 bg-blue-400/50 rounded-full animate-ping"></div>
      </div>
    </div>
  );

  const DevicePreview = ({ icon: Icon, label, isActive }) => (
    <div className={`flex items-center space-x-2 px-3 py-2 rounded-lg transition-all duration-300 ${
      isActive ? 'bg-blue-500/20 text-blue-300' : 'bg-slate-700/30 text-slate-400 hover:bg-slate-700/50 hover:text-slate-300'
    }`}>
  ...
  return (
    <section ref={sectionRef} className="min-h-screen py-12 sm:py-16 lg:py-24 relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-blue-900/40 to-purple-900/40">
        ...
        <div className="absolute bottom-20 right-10 sm:right-20 w-48 h-48 sm:w-80 sm:h-80 bg-purple-500/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }}></div>
        <div className="absolute top-1/3 right-1/4 w-32 h-32 sm:w-64 sm:h-64 bg-cyan-500/6 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '4s' }}></div>
        <div className="absolute bottom-1/3 left-1/4 w-40 h-40 sm:w-72 sm:h-72 bg-violet-500/8 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '6s' }}></div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`text-center mb-12 sm:mb-16 lg:mb-20 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mt-8 opacity-75">
            <DevicePreview icon={Monitor} label="Desktop" isActive={true} />
            <DevicePreview icon={TabletSmartphone} label="Tablet" isActive={false} />
            <DevicePreview icon={Smartphone} label="Mobile" isActive={false} />
          </div>
        </div>
            ...
            >
              <div className="space-y-4 sm:space-y-6">
                <p className="text-slate-300 leading-relaxed text-sm sm:text-base lg:text-lg">
                  With <span className="text-blue-400 font-semibold">2+ years of development experience</span>, I've successfully delivered <span className="text-purple-400 font-semibold">15+ production applications</span> using cutting-edge technologies.
                </p>
            ...
            >
              <div className="space-y-4 sm:space-y-6">
                <div className="p-4 sm:p-6 bg-gradient-to-r from-emerald-500/15 to-teal-500/15 rounded-xl lg:rounded-2xl border border-emerald-500/30">
                  ...
                  <p className="text-emerald-300 font-semibold mb-2 text-sm sm:text-base">Rai University • 2024 - 2028</p>
                    ...
                    <div className="p-4 sm:p-6 bg-gradient-to-r from-yellow-500/15 to-orange-500/15 rounded-xl lg:rounded-2xl border border-yellow-500/30">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center p-3 sm:p-4 bg-yellow-500/10 rounded-xl border border-yellow-500/20">
                          ...
                          <span className="text-white font-medium text-sm sm:text-base">Certificate of Tech Expo 2025</span>
                        </div>
                        <div className="flex items-center p-3 sm:p-4 bg-blue-500/10 rounded-xl border border-blue-500/20">
                          ...
                          <span className="text-white font-medium text-sm sm:text-base">Full Stack Development Specialization</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {skillCategories.map((category) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-xs sm:text-sm font-medium transition-all duration-300 ${
                ...
                <div className="mt-6 p-4 bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-xl border border-purple-500/20">
                  <p className="text-slate-300 text-sm leading-relaxed text-center">
                    <span className="text-purple-400 font-semibold">9 core technologies</span> mastered with <span className="text-pink-400 font-semibold">continuous learning</span> approach
                  </p>
                </div>
              </div>
            </AboutCard>
